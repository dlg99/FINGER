...
struct PredictStats {
    size_t searches_exact = 0;
    size_t searches_apprx = 0;
};
...
struct HNSWFinger {
    ...
    struct Searcher : SetOfVistedNodes<unsigned short int> {
        ...
        void setup_appx_results_containers() {
          ...
          appx_dist.resize(hnsw->graph_l0_finger.max_degree % 16 == 0 ? hnsw->graph_l0_finger.max_degree :
                           (hnsw->graph_l0_finger.max_degree / 16 + 1) * 16, 0);
...
*/
        ...
        void compute_query_projection(float *query) {
          //uint32_t tmp;
          hnsw->graph_l0_finger.finger.compute_projection_information(query, query_projection.data(), query_norm,
                                                                      query_squared_norm);
        ...
        max_heap_t &search_level(const feat_vec_t &query, index_type init_node, index_type efS, index_type level, PredictStats &stats) {
          return hnsw->search_level(query, init_node, efS, level, *this, stats);
        }

        max_heap_t &predict_single(const feat_vec_t &query, index_type efS, index_type topk, index_type num_rerank, PredictStats &stats) {
          return hnsw->predict_single(query, efS, topk, *this, num_rerank, stats);
        }
    };
    ...
    max_heap_t &predict_single(const feat_vec_t &query, index_type efS, index_type topk, Searcher &searcher,
                               index_type num_rerank, PredictStats &stats) const {
      index_type curr_node = this->init_node;
      auto &G1 = graph_l1;
      auto &G0 = feature_vec;
      // specialized search_level for level l=1,...,L because its faster for efS=1
      stats.searches_exact++;
      ...
      searcher.search_level(query, curr_node, std::max(efS, topk), 0, stats);
