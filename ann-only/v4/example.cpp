...
void run_dense(std::string data_dir, char *model_path, index_type M, index_type efC, index_type max_level, int threads,
               int efs) {
  // data prepare
  ...
  //indexer.graph_l0_finger.print_projection_matrix();
  ...
  pecos::ann::PredictStats stats;
  for (int repeat = 0; repeat < 10; repeat++) {
    double inner_latency = 0.0;

    for (index_type idx = 0; idx < num_data; idx++) {
      StopW stopw = StopW();
      auto ret_pairs = indexer.predict_single(X_tst.get_row(idx), efs, topk, searcher, num_rerank, stats);
  ...
  std::cout << "Stats after latency run: searches_exact = " << stats.searches_exact
      << ", searches_apprx = " << stats.searches_apprx << std::endl;

  double recall = 0.0;

  stats.searches_exact = 0;
  stats.searches_apprx = 0;
  ...
  std::cout << "Stats after recall run: searches_exact = " << stats.searches_exact
            << ", searches_apprx = " << stats.searches_apprx << std::endl;
...
int main(int argc, char **argv) {
  ...
  if (space_name.compare("l2") == 0) {
    run_dense<pecos::drm_t, pecos::ann::FeatVecDenseL2Simd<float>>(data_dir, model_path, M, efC, max_level, threads,
                                                                   efs);
  } else if (space_name.compare("angular") == 0) {
    run_dense<pecos::drm_t, pecos::ann::FeatVecDenseL2Simd<float>>(data_dir, model_path, M, efC, max_level, threads,
                                                                   efs);
  }
